...
trait CarRentalPriceTrait{
    ...
    private function calcPrice(){
        ...
        Log::channel('stdout')->info("CarRentalPrice calcPrice age_ranges => ".var_export($age_ranges,true));
        $age_range_arr = explode("-", $this->age_range);
        $ar_key = array_search($age_range_arr,$age_ranges);
        Log::channel('stdout')->info("CarRentalPrice calcPrice ar_key => ".var_export($ar_key,true));
        $company_arr = $this->data['companies_data'][$this->company_name];
        Log::channel('stdout')->info("CarRentalPrice calcPrice company_arr => ".var_export($company_arr,true));
        $car_arr = $this->data['companies_data'][$this->company_name]['cars'][$this->car_name];
        Log::channel('stdout')->info("CarRentalPrice calcPrice car_arr => ".var_export($car_arr,true));
        ...
        Log::channel('stdout')->info("CarRentalPrice calcPrice price_company_part => ".var_export($price_company_part,true));
        $price_car_part = $car_arr['day_price'] + ($car_arr['day_price'] * $company_arr['day_price_additional_pcg'][$ar_key] / 100);
        Log::channel('stdout')->info("CarRentalPrice calcPrice price_car_part => ".var_export($price_car_part,true));
        ...
        Log::channel('stdout')->info("CarRentalPrice calcPrice dti_diff => ".var_export($dti_diff,true));
        $this->total_price = ($price_company_part + $price_car_part) * (float)$dti_diff->d;
        Log::channel('stdout')->info("CarRentalPrice calcPrice total_price => ".var_export($this->total_price,true));
    }
}
